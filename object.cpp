...
//  if(obj->gc==0) { 
    // no group spotted so we add one
//    obj->gc=1;
//    gi = 0;
//    nogrp=true;
//    }
//  //  blit::debugf("object: g-%d v-%d t-%d n-%d\n",obj->gc,obj->vc,obj->tc,obj->nc);
...
////  blit::debugf("object: Second pass\n");
...
//  //  blit::debugf("object: rewind file\n");
  ...
  while (p < eof) {
    ...
    if (!strcmp(token, "g")) {
 //       //  blit::debugf("object: token - group\n");
      gi++;
    }

    if (!strcmp(token, "f")) {
//      //  blit::debugf("object: token - face\n");
...
//    //  blit::debugf("object: token - face - added to group\n");

    }

    if (!strcmp(token, "v")) {
//              //  blit::debugf("object: token - vertex\n");
    ...
    if (!strcmp(token, "vn")) {
//              //  blit::debugf("object: token - normal\n");
    ...
    if (!strcmp(token, "vt") && obj->tc>0) {
//      //  blit::debugf("object: token - uv pt\n");
...
//  //  blit::debugf("object: Second pass - Groups\n");
...
//    }
  ...
  while (p < eof) {
    ...
    if (!strcmp(token, "g")) {
      gi++; fi = 0;
//      //  blit::debugf("object: group found\n");
    }

    if (!strcmp(token, "f")) {
//            //  blit::debugf("object: token - face - add %d to group %d\n",fi,gi);
...
////  blit::debugf("object: %f %f %f  %f %f %f  %f %f %f\n",f->v[0],f->t[0],f->n[0]);
...
////  blit::debugf("object: Update Bounding box\n");
